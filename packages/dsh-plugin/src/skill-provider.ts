...
const DSH_SKILL_BRIDGE = [
  ...
  "Build novel projects at the workspace root or in a single top-level book directory (a first-level subdirectory directly containing 正文/ or 追踪/); the workbench recognizes both.",
...
const DSH_SKILL_OVERRIDES: Readonly<Partial<Record<string, string>>> = {
  ...
  "story-setup": "Initialize or validate novel project data only. DSH already supplies Skills, Roles, hooks, tools, permissions, sessions, and UI; never deploy Claude/OpenCode/Codex/Antigravity/ZCode/OpenClaw/Reasonix files or a .story-deployed marker. Build at the workspace root or in one top-level book directory (a first-level subdirectory directly containing 正文/ or 追踪/); the workbench recognizes both.",
...
const DSH_DRAMA_OVERRIDES: Readonly<Partial<Record<string, string | ((skillRoot: string) => string)>>> = {
  ...
  "short-drama-produce": (skillRoot) => `Use an upstream adapter only after the creator explicitly confirms the exact current job. Its source must be the current creator-first Markdown and its output belongs under 剧集/<EP>/制作成果/. DSH permissions and approval UI remain authoritative. When oh_story_production is visible, register the previewed job with track_job after the confirmation is valid and before run — never at prepare time — passing the same job ID, target, modality and count the creator just approved, and the confirmed output filenames via track_job outputs. DeepSeek generates no media: every image, video, or music result comes from a provider adapter. The bundled adapters are ${dramaAdapterSummary()}; they are registered for this DSH host in ${dramaAdapterConfigPath(skillRoot).path} — pass that file as --adapter-config unless the creator names another config (${DRAMA_ADAPTER_CONFIG_ENV}). Credentials are read only from the DSH host process environment. Before preparing a paid job, name the adapter that will run it; if its variables are missing, tell the creator exactly which ones to export before starting DSH instead of failing inside run. Never read, print, or write credential values. For ComfyUI adapters a per-job workflow goes in the job parameters as parameters.workflow (a workflow name resolved against COMFYUI_WORKFLOW_DIR, or an absolute path); a top-level workflow key is rejected by prepare. Every output filename must contain the job ID and the target object ID so the workbench can reconcile the result — prepare does not enforce this. When generating an image for a shot, if a character or scene in that shot already has a VISUAL-* locked reference portrait in 视觉设定.md produced under 剧集/<EP>/制作成果/, write its project-relative path into the job parameters input_image as the img2img reference (the runner resolves it against the run snapshot project_root, so the file must already be a confirmed job input; with multiple candidates use only the lead character's single image because the ComfyUI workflow exposes a single __INPUT_IMAGE__ input). Dialogue dubbing is generated per shot through oh_story_comfyui with the creator-configured TTS voice workflow (modality-agnostic submit/download; see plugin docs/comfyui.md "短剧台词配音"): outputs land at 剧集/<EP>/配音/<SHOT-ID>.wav as mono 16-bit 44100 Hz WAV, and every filename must carry its SHOT-ID so the 串播 view auto-links it. When that tool is absent or no voice workflow is configured, say so instead of inventing audio. The upstream video-recap voiceover contract (tts_meta.json + assemble) does not apply to short drama — never mix them.`,
...
const DSH_GAME_BRIDGE = [
  ...
  "A finished image under game-adaptations/<project>/art/ is only material-ready, never integration-complete: game-build must explicitly wire each locked-in image into build/app/ (for example the title-screen background, scene cards, or character portrait slots), and the reference paths must be recorded in both the game code and design/ART_DIRECTION.md.",
  "After wiring art, re-run the game-qa six-item minimum QA (launch, render, input, coreLoop, outcome, restart) with real rendering evidence for every scene that references art, and record the on-screen art evidence in the corresponding qa/verification.json checks.",
  "Every wired image keeps one stable `- ID：ART-*` line in design/ART_DIRECTION.md, following the short-drama VISUAL-* stable-ID practice; game code references the file registered under that ID.",
